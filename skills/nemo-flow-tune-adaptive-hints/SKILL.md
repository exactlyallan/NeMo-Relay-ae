---
name: nemo-flow-tune-adaptive-hints
description: Consume NeMo Flow adaptive hints, predictions, latency sensitivity, ACG diagnostics, or tool-parallelism guidance safely in application logic after the adaptive plugin is already configured
...
plugin from scratch. Use `nemo-flow-tune-performance` or
`nemo-flow-tune-adaptive-config` first.
